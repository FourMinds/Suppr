...
(function (root, factory) {
  ...
  let Imgur = function (options) {
    ...
    this.dropzone = document.querySelectorAll('.dropzone-profile');
  ...
  Imgur.prototype = {
    ...
    createDragZone: function () {
      ...
      p   = this.createEls('p', {className:'profile-drop-text'}, 'Drag your images here or click in this area.');
        ...
        zone.addEventListener(event, function (e) {
          if (e.target && e.target.nodeName === 'INPUT' && e.target.type === 'file') {
            if (event === 'dragleave' || event === 'drop') {
              e.target.parentNode.classList.remove('dropzone-profile-dragging');
            } else {
              e.target.parentNode.classList.add('dropzone-profile-dragging');
            }
          }
        }, false);
      });
